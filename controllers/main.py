...
class WebsiteSaleCustom(WebsiteSale):
    ...
    def payment_transaction(self, **post):
        ...
        y manejar correctamente el flujo de pago
        """
        ...
        if not order.partner_id or order.partner_id.id == request.website.partner_id.id:
            # Si no hay partner o es el mismo que el website, crear uno nuevo
            # o usar el partner de la sesión
            ...
            if session_partner_id:
                ...
                if partner.exists():
                    ...
                    return {'error': 'Partner de sesión no válido'}
            else:
                return {'error': 'No hay partner válido asignado'}
        
        # Ahora llamar al método padre con el partner correctamente asignado
        try:
            return super(WebsiteSaleCustom, self).payment_transaction(**post)
        except Exception as e:
            # Si hay algún error, devolver información útil
            return {
